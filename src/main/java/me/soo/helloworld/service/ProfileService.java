...
public class ProfileService {
    ...
    public UserProfile getUserProfile(String userId) {
        UserDataOnProfile profile = profileMapper.getUserProfileData(userId)
                .orElseThrow(() -> new InvalidRequestException("존재하지 않거나 정보가 올바르게 등록되지 않은 사용자의 경우 프로필 조회가 불가능합니다. 나중에 다시 시도해주세요."));

        List<RecommendationDataForProfile> recommendations = recommendationService.getRecommendationsByUserId(userId);
